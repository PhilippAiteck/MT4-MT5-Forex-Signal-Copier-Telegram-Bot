...
# Base SQLite pour l'historique des trades
...
# Helper Functions
def init_db():
    """Crée la base SQLite et la table deals si elles n'existent pas."""
    ...
    conn.commit()
    conn.close()
    logger.info(f"SQLite initialisée sur {DB_PATH}")
...
def save_deals_to_db(raw_history, account_id: str) -> int:
    """
    Enregistre les deals MetaApi dans la base SQLite.
    Retourne le nombre de deals insérés/mis à jour.
    """
    # 👉 Extraire la vraie liste de deals
    if isinstance(raw_history, dict) and "deals" in raw_history:
        deals = raw_history["deals"]
    else:
        deals = raw_history  # au cas où MetaApi change de format plus tard
    ...
    for deal in deals:
        if not isinstance(deal, dict):
            logger.warning(f"Deal non dict: {deal}")
            continue

        deal_id = deal.get("id")
        if not deal_id:
            logger.warning(f"Deal sans id: {deal}")
            continue
        ...
        stop_loss = deal.get("stopLoss")
        take_profit = deal.get("takeProfit")

        try:
            cursor.execute(
                """
                INSERT OR REPLACE INTO deals (
                    id, platform, type, symbol, time, broker_time,
            ...
            logger.error(f"Échec insertion DB pour deal {deal_id}: {e}")

    conn.commit()
    conn.close()
    logger.info(f"{inserted} deals insérés/mis à jour dans SQLite")
    return inserted

def get_trade_report_from_db(days: int = 210, symbol: str = None):
    """Calcule un petit rapport de trading à partir de la table deals."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()

    # Filtre de date : derniers X jours
    ...
    nb_deals, pnl_total = cursor.fetchone() or (0, 0)

    # 2) Winrate : on compte deals de sortie (DEAL_ENTRY_OUT) avec profit > 0
    cursor.execute(
        f"""
        SELECT 
    ...
    cursor.execute(
        ...
        FROM deals
        WHERE time >= ?
        {symbol_filter.replace("AND symbol = ?", "")}  -- on enlève le filtre si déjà symbol fixé
          AND volume IS NOT NULL
        GROUP BY symbol
        ORDER BY pnl DESC
        LIMIT 10
    ...
    return {
        "nb_deals": nb_deals or 0,
        "pnl_total": pnl_total or 0,
        "wins": wins or 0,
        "losses": losses or 0,
        "winrate": winrate,
...
async def ConnectGetTradeHistory(update: Update, context: CallbackContext) -> None:
    """Récupère l'historique des deals MetaApi et les enregistre dans SQLite."""
    ...
    try:
        ...
        # 🟢 Période : on va prendre large pour les tests, par ex. 365 jours
        days = 1825
        start_time = datetime.utcnow() - timedelta(days=days)
        end_time = datetime.utcnow()

        logger.info(f"Fetching deals from {start_time} to {end_time}")
        raw_history = await connection.get_deals_by_time_range(start_time, end_time)

        # Ici, raw_history est un dict avec 'dealsLength' et 'deals'
        if isinstance(raw_history, dict) and "deals" in raw_history:
            deals = raw_history["deals"]
            logger.info(
                f"MetaApi a retourné dealsLength={raw_history.get('dealsLength')} / len(deals)={len(deals)}"
            )
        else:
            deals = raw_history
            logger.info(f"MetaApi a retourné une liste brute de {len(deals)} deals")

        if not deals:
            update.effective_message.reply_text(
                "ℹ️ Aucun deal trouvé sur la période demandée."
            )
            return

        # 🟢 Sauvegarde dans SQLite
        inserted = save_deals_to_db(raw_history, ACCOUNT_ID)

        update.effective_message.reply_text(
            f"✅ Historique synchronisé dans la base SQLite.\n"
            f"Période : {start_time.date()} → {end_time.date()}\n"
            f"Deals récupérés (len(deals)) : {len(deals)}\n"
            f"Deals insérés/maj en DB : {inserted}"
        )

    except Exception as error:
        logger.error(f"Error in ConnectGetTradeHistory: {error}")
        update.effective_message.reply_text(
            f"❌ Impossible de récupérer l'historique des trades.\nErreur: {error}"
        )
...
def GetTradeReport(update: Update, context: CallbackContext):
    """Envoie un mini rapport texte basé sur la DB SQLite."""
    # Exemple : /report 30 XAUUSD
    args = context.args
    days = 210
    symbol = None
    ...
    report = get_trade_report_from_db(days=days, symbol=symbol)

    msg = f"📊 Rapport de trading sur {report['period_days']} jours"
    if report["symbol"]:
        msg += f" (symbole: {report['symbol']})"
    msg += "\n\n"
    msg += f"• Nombre de deals : {report['nb_deals']}\n"
    msg += f"• PnL total : {report['pnl_total']}\n"
    msg += f"• Winrate : {report['winrate']:.1f}% ({report['wins']} wins / {report['losses']} losses)\n\n"

    msg += "Top symboles (PnL):\n"
    for sym, n, pnl in report["by_symbol"]:
        msg += f"  - {sym} : {pnl} ({n} deals)\n"

    update.effective_message.reply_text(msg)

def setup_dispatcher(dp):
    """Enregistre tous les handlers Telegram sur le dispatcher donné."""
    ...
    # command to get trade report from the database
    dp.add_handler(CommandHandler("report", GetTradeReport))

    # command to get trade history from MetaApi and save to SQLite
    #dp.add_handler(CommandHandler("trade_history", TradeHistory))
...
"""
def TradeHistory(update: Update, context: CallbackContext):
    ""Synchronise l'historique (nouveaux deals) et affiche un résumé.""

    if not (update.effective_message.chat.username == TELEGRAM_USER):
        update.effective_message.reply_text("You are not authorized to use this bot! 🙅🏽‍♂️")
        return

    update.effective_message.reply_text("🔄 Synchronisation de l'historique en cours...")

    try:
        inserted = incremental_sync_history()
        update.effective_message.reply_text(
            f"✅ Synchronisation terminée.\n"
            f"🆕 Nouveaux trades ajoutés : {inserted}"
        )
    except Exception as e:
        logger.error(f"/trade_history error: {e}")
        update.effective_message.reply_text(f"❌ Erreur lors de la synchronisation : {e}")
"""
...
# async def main() -> None:
    ...
    init_db()   # 👉 on s’assure que la DB est prête
