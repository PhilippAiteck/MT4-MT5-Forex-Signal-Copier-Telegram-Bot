            ...
            try:
                ...
                if(trade['OrderType'] == 'Buy'):
                    ...
                    if(account_information['balance'] <= 499):
                        ...
                        result = await connection.create_market_buy_order(trade['Symbol'], round(trade['PositionSize'] / 2, 2), trade['Entry'])

                    elif(account_information['balance'] > 499 and account_information['balance'] < 1000):
                        trade['PositionSize'] = 0.04
                        result = await connection.create_market_buy_order(trade['Symbol'], round(trade['PositionSize'] / 3, 2), trade['Entry'])
                ...
                # executes sell market execution order
                    ...
                    if(account_information['balance'] <= 499):
                        trade['PositionSize'] = 0.02
                        result = await connection.create_market_sell_order(trade['Symbol'], round(trade['PositionSize'] / 2, 2), trade['Entry'])

                    elif(account_information['balance'] > 499 and account_information['balance'] < 1000):
                        trade['PositionSize'] = 0.04
                        result = await connection.create_market_sell_order(trade['Symbol'], round(trade['PositionSize'] / 3, 2), trade['Entry'])
